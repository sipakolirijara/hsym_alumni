...
class _FinSecDashboardScreenState extends State<FinSecDashboardScreen> {
  ...
  Future<void> _fetchData() async {
    final result = await FinSecService.getDashboard();
            ...
            children: [
              Row(
                children: [
                  ...
                  Expanded(child: _buildStatCard('Pending', '${_stats?['pending_tasks'] ?? 0}', Icons.pending_actions, Colors.orange)),
                ],
              ),
              const SizedBox(height: 16),
              _buildStatCard('Total Expected Revenue (NGN)', '${_stats?['total_revenue'] ?? 0}', Icons.account_balance, AppTheme.brandPrimary),
              const SizedBox(height: 32),
              const Text('Financial Oversight', style: TextStyle(color: Colors.black87, fontSize: 18, fontWeight: FontWeight.bold)),
              const SizedBox(height: 24),
              Wrap(
                spacing: 24, runSpacing: 24, alignment: WrapAlignment.start,
                children: [
                  _buildCircularAction(context, Icons.account_balance_wallet, 'Ledger', () => context.push('/treasurer/contributions')), // Shared with Treasurer
                  _buildCircularAction(context, Icons.bar_chart, 'Reports', () => ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Reports module coming soon')))),
                ],
              ),
              const SizedBox(height: 32),
